...
import type { TranscriptItem, ChatMessage } from "./types";
...
export default function AskAiTab({ isActive, initialQuestion, onClearInitialQuestion, transcript, resourceId, token, onSeek }: AskAiTabProps) {
  ...
  const audioTasks = backgroundTasks.filter(
    (task) => task.surface === "audio" && task.ownerKey === `audio:${resourceId}`,
  );
  ...
  useEffect(() => {
      ...
      for (const task of audioTasks) {
        ...
        const updated: ChatMessage = {
          ...
          sources: task.sources.length > 0
            ? task.sources as ChatMessage["sources"]
            : existing?.sources,
          details,
        };
        next = existing
    ...
    setLoading(audioTasks.some((task) => task.status === "running"));
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data && data.length > 0) {
            ...
            setMessages((prev) => {
              // Preserve in-flight local chat state when history finishes after
              // the user has already started a streamed request.
              if (prev.length > 0) {
                return prev;
              }
              return mapped;
  ...
  useEffect(() => {
    const fetchSuggestions = async () => {
      ...
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          console.log("[AskAiTab] loaded saved suggested questions", {
            resourceId,
  ...
  const reloadSuggestions = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        console.log("[AskAiTab] regenerated suggested questions", {
          resourceId,
          count: Array.isArray(data.questions) ? data.questions.length : 0,
        });
        if (data.questions && data.questions.length > 0) {
  ...
  const handleRegenerate = (msg: ChatMessage) => {
    ...
    if (!msg.query || loading || !resourceId || !token) return;
  ...
  const handleSendMessage = async (textToSend: string, regenerateMsgId?: string) => {
    if (!textToSend.trim() || loading || !resourceId || !token) return;

    let activeSessionId = sessionId;
    if (!activeSessionId) {
      const title = textToSend.trim().length > 25 ? textToSend.trim().substring(0, 25) + '...' : textToSend.trim();
    ...
    const ownerKey = `audio:${resourceId}`;
    startTask({
      id: `audio-task:${requestSessionId}:${aiMessageId}`,
      ownerKey,
      surface: "audio",
      query: textToSend.trim(),
      ...
      const processEvent = (payload: any) => {
        ...
        if (payload.type === "final") {
          complete({
            ...
            details: buildResponseDetails(payload, textToSend.trim()) as Record<string, unknown>,
          });
          return;
        }
  ...
  const handleContinueAnswer = async (messageId: string) => {
    ...
    startTask({
      id: `audio-continue:${activeSessionId}:${messageId}:${Date.now()}`,
      ownerKey: `audio:${resourceId}`,
      surface: "audio",
  ...
  const clearChat = async () => {
    if (resourceId) clearOwner(`audio:${resourceId}`);
    if (!sessionId || !token) {
      setMessages([]);
      setError(null);
      return;
  ...
  useEffect(() => {
    if (isActive && messages.length > 0) {
      // Use requestAnimationFrame to ensure DOM is painted after becoming visible
  ...
  const askAiContent = (
    <div className="flex-1 flex flex-col space-y-6 h-full min-h-0">
      ...
      {messages.length === 0 ? (
        <div className="space-y-6 py-4 flex-1 overflow-y-auto no-scrollbar min-h-0">
          <div className="bg-neutral-50/70 p-6 rounded-xl border border-neutral-100 text-center space-y-3">
            ...
            <p className="text-base font-bold text-neutral-705">Ask anything about the Standup</p>
            <p className="text-sm text-neutral-400 max-w-sm mx-auto">
              Ask about task assignments, specific timelines, tech stacks (Recharts, Figma status), or owner priorities.
            </p>
          </div>
                ...
                >
                  {suggestions
                    .slice(
                      ...
                      suggestionPage * SUGGESTIONS_PER_PAGE + SUGGESTIONS_PER_PAGE
                    )
                    .map((sug, idx) => (
                      <button
        ...
        <div className="flex-1 space-y-4 overflow-y-auto no-scrollbar pr-1 min-h-0">
          {visibleMessages.map((m) => (
            <div
              key={m.id}
              className={`flex w-full ${m.sender === 'user' ? 'justify-end' : 'justify-start'} animate-fade-in`}
            >
              {m.sender === 'ai' && (
                    ...
                    <div className="rounded-[30px] rounded-tl-none px-6 py-5 backdrop-blur-md border border-white/70 dark:border-white/10 bg-slate-50/50 dark:bg-slate-800/50 shadow-[0_24px_60px_-28px_rgba(148,163,184,0.35)]">
                      ...
                      <div className="text-[15px] leading-relaxed text-slate-700 dark:text-slate-200">
                        <TypewriterMessage
                          content={m.text || ''}
                          msgId={m.id}
                          animate={audioTasks.some((task) => task.targetMessageId === m.id && task.status === "running")}
                          streaming={audioTasks.some((task) => task.targetMessageId === m.id && task.status === "running")}
                          timelineStartedAt={audioTasks.find((task) => task.targetMessageId === m.id)?.firstContentAt}
                      ...
                      {m.details?.canContinue && messages[messages.length - 1]?.id === m.id && (
                        <button
                          type="button"
                          disabled={loading}
                          onClick={() => handleContinueAnswer(m.id)}
                      ...
                      {m.sources && m.sources.length > 0 && (
                        <SourceList sources={m.sources} theme={document.documentElement.classList.contains("dark") ? "dark" : "light"} />
                      )}
                      {/* ── Action Bar ── */}
                      <div className="mt-4 flex items-center justify-between gap-3 pt-3 border-t border-slate-200/60 dark:border-white/10">
                        ...
                        <div className="flex items-center gap-3 flex-wrap">
                          {m.details?.processingTimeMs != null ? (
                            <span className="inline-flex items-center gap-1 text-[10px] font-medium text-slate-500 dark:text-gray-400">
                              <Clock size={11} />
                              Latency: {m.details.processingTimeMs < 1000
                                ? `${Math.round(m.details.processingTimeMs)}ms`
                                : `${(m.details.processingTimeMs / 1000).toFixed(2)}s`}
                            </span>
                          ) : null}
                          {(m.sources?.length ?? 0) > 0 ? (
                            <span className="inline-flex items-center gap-1 text-[10px] font-medium text-slate-500 dark:text-gray-400">
                              <BarChart2 size={11} />
                              Scanned Vector: {(m.details as any)?.scannedVectorCount ?? m.sources!.length}
                            </span>
                          ) : null}
                          {m.details?.retrievalStrategy ? (
                            <span className="inline-flex items-center gap-1 text-[10px] font-medium text-slate-500 dark:text-gray-400">
                              <Sparkles size={11} />
                              Mode: {m.details.retrievalStrategy}
                            </span>
                          ) : null}
                          {!m.details?.processingTimeMs && !(m.sources?.length ?? 0) && !m.details?.retrievalStrategy && (
                            <span className="text-[10px] text-slate-400 font-mono select-none">{m.timestamp}</span>
                          )}
                        </div>
                        ...
                        <div className="flex items-center gap-0.5">
                          <button
                            ...
                            className={`p-1.5 rounded-lg transition-colors cursor-pointer hover:bg-slate-100 dark:hover:bg-white/10 hover:text-slate-800 dark:hover:text-white text-slate-400 dark:text-gray-400 ${reactions[m.id] === 'like' ? 'text-emerald-500 bg-emerald-500/10' : ''}`}
                            title="Helpful"
                          >
                            <ThumbsUp size={13} className={reactions[m.id] === 'like' ? 'fill-emerald-400' : ''} />
                          </button>
                          <button
                            onClick={() => toggleReaction(m.id, 'dislike')}
                            className={`p-1.5 rounded-lg transition-colors cursor-pointer hover:bg-slate-100 dark:hover:bg-white/10 hover:text-slate-800 dark:hover:text-white text-slate-400 dark:text-gray-400 ${reactions[m.id] === 'dislike' ? 'text-rose-500 bg-rose-500/10' : ''}`}
                            title="Not helpful"
                          >
                            <ThumbsDown size={13} className={reactions[m.id] === 'dislike' ? 'fill-rose-400' : ''} />
                          </button>
                          <button
                          ...
                          <div className="relative inline-block">
                            <button
                              onClick={(e) => { e.stopPropagation(); setOpenDropdownId(openDropdownId === m.id ? null : m.id); }}
                              className={`p-1.5 rounded-lg transition-colors cursor-pointer hover:bg-slate-100 dark:hover:bg-white/10 hover:text-slate-800 dark:hover:text-white text-slate-400 dark:text-gray-400 ${openDropdownId === m.id ? 'bg-slate-100 dark:bg-white/10 text-slate-800 dark:text-white' : ''}`}
                              title="More options"
                            >
                            ...
                            {openDropdownId === m.id && (
                              <>
                                ...
                                <div className="absolute right-0 bottom-full mb-1 w-44 rounded-xl p-1 z-50 flex flex-col backdrop-blur-md shadow-lg bg-white dark:bg-neutral-800/95 border border-slate-200 dark:border-white/10">
                                  <button
                                    onClick={() => { exportMsgToMarkdown(m); setOpenDropdownId(null); }}
                                  ...
                                  <button
                                    onClick={() => { exportMsgToPdf(m); setOpenDropdownId(null); }}
                                    className="flex items-center gap-2 text-left w-full px-2.5 py-1.5 rounded-lg text-[11px] font-semibold transition-colors hover:bg-slate-50 dark:hover:bg-white/10 text-slate-700 dark:text-gray-200 cursor-pointer"
                                  >
              ...
              {m.sender === 'user' && (
                <div className="flex flex-col items-end max-w-[80%]">
                  ...
                  <span className="text-[10px] mt-1.5 text-neutral-400 font-mono select-none pr-1">{m.timestamp}</span>
                </div>
              )}
            </div>
          ))}

          {loading && hasPendingEmptyAiMessage && (
      ...
      {messages.length > 0 && suggestions.length > 0 && (
        <div className="shrink-0 mb-2 bg-white dark:bg-slate-800 border border-gray-200 dark:border-white/10 rounded-xl overflow-hidden shadow-xs">
          ...
          {showSuggestionsDrawer && (
            <div className="p-3 bg-white dark:bg-slate-800 grid grid-cols-1 gap-2 max-h-48 overflow-y-auto no-scrollbar custom-scrollbar">
      ...
      >
        <input
          ...
          placeholder="Ask me anything about audio..."
          disabled={loading}
        ...
        <button
          ...
          className={`p-3.5 rounded-lg transition-all flex items-center justify-center shrink-0 ${!inputText.trim() || loading
