...
export function SummaryView({ transcript: _transcript, summary: initialSummary, resourceId, token, chapters, subchapters, onSummaryUpdated, onPendingReindexChange, pendingReindex, onSeek }: SummaryViewProps) {
  ...
  if (error) {
    return (
      <div className="flex-1 flex flex-col h-full bg-white dark:bg-[#1e1f22] relative">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {summary && !loading && (
          <div className="flex items-center space-x-3">
            <button
              onClick={handleCopy}
              className="p-2 hover:bg-slate-100 rounded-lg text-slate-600 hover:text-slate-900 transition flex items-center space-x-1.5 text-sm font-bold cursor-pointer"
              title="Copy Summary"
            >
      ...
      {loading ? (
        ...
        <div className="py-20 flex flex-col items-center justify-center space-y-5">
          <div className="relative w-12 h-12">
            ...
            <div className="absolute inset-0 rounded-full border-3 border-slate-800 border-t-transparent animate-spin"></div>
          </div>
          <div className="text-center space-y-2">
            <p className="text-base font-bold text-slate-800">Analyzing meeting dialogue...</p>
            <p className="text-sm text-slate-400">Gemini is drafting an executive breakdown</p>
          </div>
        </div>
        )
      ) : error ? (
        ...
        wasSavedLoad ? (
          <SavedContentReveal>
            <div className="bg-slate-50/50 dark:bg-slate-800/50 rounded-2xl border border-slate-100/80 dark:border-white/10 p-8 overflow-y-auto no-scrollbar flex-1 min-h-0 leading-relaxed font-normal animate-fade-in duration-500">
        ...
        <div className="text-center py-24 bg-slate-50 rounded-xl border border-dashed border-slate-200">
          ...
          <p className="text-lg font-bold text-slate-700">Ready to analyze meeting</p>
          <p className="text-sm text-slate-400 max-w-md mx-auto mt-2">
            Click generate to get an executive breakdown.
          </p>
          <button
            onClick={() => fetchSummary(false)}
            className="mt-6 px-6 py-2.5 bg-slate-800 hover:bg-slate-900 text-white font-bold text-sm rounded-full transition cursor-pointer"
          >
