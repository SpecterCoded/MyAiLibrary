    ...
    >
      ...
      <div className="absolute top-3 right-3 flex items-center gap-1.5 bg-white/80 dark:bg-slate-800/80 backdrop-blur-md px-2 py-1.5 rounded-full border border-slate-200/50 dark:border-white/10 z-10 shadow-xs">
        <button
          type="button"
          onClick={zoomIn}
          className="p-1 hover:bg-slate-150 dark:hover:bg-slate-700 rounded-full text-slate-600 dark:text-slate-350 cursor-pointer font-bold text-xs w-6 h-6 flex items-center justify-center border-none outline-none bg-transparent"
          title="Zoom In"
        >
          ＋
        </button>
...
export function MindMapView({ resourceId, token, initialMindmap, onMindmapGenerated }: MindMapViewProps) {
  ...
  const fetchMindMap = async (forceRegenerate = false) => {
    ...
    try {
      if (!forceRegenerate) {
        ...
        if (data) {
          setMindmap(data);
          onMindmapGenerated?.(data); // notify parent so next visit starts in sparkle mode
        } else {
  ...
  if (!mindmap) {
    return (
      <div className="text-center py-24 bg-neutral-50 rounded-xl border border-dashed border-neutral-200 mx-6">
  ...
  const mindmapContent = (
    <div className="space-y-6 animate-fade-in flex-1 flex flex-col min-h-0 px-6 pb-24 bg-white dark:bg-[#1e1f22]">
      <div className="flex items-center justify-between border-b border-neutral-100 pb-3.5 shrink-0 mt-6">
