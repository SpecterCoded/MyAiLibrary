...
function formatSessionDate(value?: string | null): string {
  ...
  // Backend stores UTC without Z suffix — add it so JS treats it as UTC
  const timestamp = value.endsWith('Z') || value.includes('+') ? value : value + 'Z';
...
function detectSuggestionKind(resource: any): 'video' | 'audio' | 'doc' {
  ...
  const localPath = String(resource?.local_path || '').toLowerCase();
  const previewUrl = String(resource?.preview_url || '').toLowerCase();
  const haystack = `${title} ${description} ${localPath} ${previewUrl}`;

  if (
    resourceType === 'video' ||
    resourceType === 'youtube' ||
    /youtube\.com|youtu\.be/.test(haystack) ||
    /\.(mp4|mov|avi|mkv|webm|m4v)\b/.test(haystack)
  ) {
    return 'video';
  }
  ...
  if (
    resourceType === 'audio' ||
    /\.(mp3|wav|m4a|aac|flac|ogg|opus)\b/.test(haystack)
  ) {
    return 'audio';
  }
...
function stripMarkdownToPreview(text: string): string {
  return text
    .replace(/```[\s\S]*?```/g, ' ')
    .replace(/`([^`]+)`/g, '$1')
    .replace(/!\[[^\]]*\]\([^)]+\)/g, ' ')
...
function buildSuggestionDescription(resource: any, kind: 'video' | 'audio' | 'doc'): string {
  const baseText = stripMarkdownToPreview(String(resource?.summary || resource?.description || ''));
  if (baseText) {
    return baseText.length > 120 ? `${baseText.slice(0, 117).trimEnd()}...` : baseText;
  }

  if (kind === 'video') return 'A concise AI-ready video overview is available in your library.';
  if (kind === 'audio') return 'An audio summary is ready with the main ideas and takeaways.';
  return 'A clean document summary is ready to open from your library.';
}

function buildSuggestionPrompt(title: string, kind: 'video' | 'audio' | 'doc'): string {
  if (kind === 'video') return `Walk me through the main moments and takeaways from "${title}".`;
  if (kind === 'audio') return `Give me the key ideas and best takeaways from "${title}".`;
  return `Summarize the most important points from "${title}".`;
}

function buildNotebookBlocksFromChat(session: ChatSession): any[] {
  const blocks: any[] = [];

  session.messages.forEach((message) => {
    const roleLabel = message.role === 'user' ? 'You' : 'AI Answer';
    blocks.push({
      type: 'heading',
      props: { level: 2 },
      content: roleLabel,
    ...
    lines.forEach((line) => {
      blocks.push({
        type: line.startsWith('- ') || line.startsWith('* ') ? 'bulletListItem' : 'paragraph',
...
function buildFolderPath(folder: any, folderById: Map<string, any>): string {
  const parts: string[] = [];
  ...
  while (current && !seen.has(current.id)) {
    seen.add(current.id);
    parts.unshift(current.name || 'Folder');
...
export default function ChatApp({ user }: ChatAppProps) {
  ...
  const previewedSuggestionResourcesRef = useRef<Set<string>>(new Set());
  ...
  useEffect(() => {
    const loadDynamicSuggestions = async () => {
      try {
        ...
        // Filter to only media types: video, audio (YouTube is marked as video)
        const mediaTypes = ['video', 'youtube', 'audio'];
        const mediaResources = data.filter((r: any) => {
          const resourceType = String(r.type || r.resource_type || '').toLowerCase();
          const localPath = String(r.local_path || '').toLowerCase();
          const title = String(r.title || '').toLowerCase();
          // Check if it's a media type by resource type or file extension
          const isMedia = mediaTypes.includes(resourceType) ||
            /\.(mp4|mov|avi|mkv|webm|m4v|mp3|wav|m4a|aac|flac|ogg|opus)$/i.test(localPath) ||
            /youtube\.com|youtu\.be/i.test(title);
          return isMedia;
        });
        if (mediaResources.length === 0) return;
        // Sort by created_at descending and pick top 3
        const sorted = mediaResources.sort((a: any, b: any) => {
          const dateA = new Date(a.created_at || 0).getTime();
          const dateB = new Date(b.created_at || 0).getTime();
          return dateB - dateA;
        });
        const mapped = sorted.slice(0, 3).map((r: any) => {
          const kind = detectSuggestionKind(r);
          return {
            resourceId: r.id,
            kind,
            title: r.title || 'Untitled',
            desc: buildSuggestionDescription(r, kind),
            btn: 'Open Summary',
            prompt: buildSuggestionPrompt(r.title || 'this resource', kind),
            summaryText: typeof r.summary === 'string' ? r.summary : '',
            resourceType: r.type || r.resource_type || kind,
          };
        });
  ...
  const handleOpenSuggestion = async (suggestion: any) => {
    const promptText = typeof suggestion?.prompt === 'string' ? suggestion.prompt : '';
    if (!suggestion?.resourceId) {
      await handleSendMessage(promptText);
      return;
    }

    const focusedResources = [{ id: suggestion.resourceId, title: suggestion.title || 'Untitled' }];
    setSelectedResources(focusedResources);

    // Always fetch summary fresh from the resource endpoint to ensure correct data
    let summaryText = '';
    try {
      const token = localStorage.getItem('access_token');
      const response = await fetch(`/resources/${suggestion.resourceId}/summary`, {
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
      if (response.ok) {
        const data = await response.json();
        summaryText = typeof data?.summary === 'string' ? data.summary.trim() : '';
      }
    } catch {
      summaryText = '';
    }

    // Fallback to pre-loaded summary if fetch failed
    if (!summaryText) {
      summaryText = typeof suggestion.summaryText === 'string' ? suggestion.summaryText.trim() : '';
    }

    if (!summaryText) {
      await handleSendMessage(promptText, focusedResources);
      return;
    }
    ...
    // Always create a NEW session for each card click
    const sessionTitle = suggestion.title || promptText || 'Library summary';
    let updatedChats = [...chats];
    let createdSessionId = '';

    try {
      const backendSession = await createChatSession(sessionTitle.length > 25 ? `${sessionTitle.substring(0, 25)}...` : sessionTitle);
      createdSessionId = backendSession.id;
      setActiveSessionId(createdSessionId);
      updatedChats = [{
        id: createdSessionId,
        title: sessionTitle.length > 25 ? `${sessionTitle.substring(0, 25)}...` : sessionTitle,
        date: formatSessionDate(backendSession.created_at),
        timeAgo: formatSessionTimeAgo(new Date().toISOString()),
        lastActivityAt: new Date().toISOString(),
        messages: [userMessage, aiMessage],
        preview: stripMarkdownToPreview(summaryText).slice(0, 120) || sessionTitle,
      }, ...updatedChats];
    } catch {
      await handleSendMessage(promptText, focusedResources);
      return;
    ...
    // Save messages to backend so they persist in history
    try {
      const token = localStorage.getItem('access_token');
      if (token && createdSessionId) {
        const saveRes = await fetch('/chat/sessions/messages', {
          method: 'POST',
          headers: {
    ...
    try {
      sessionStorage.removeItem(`streamed-${aiMessage.id}`);
    } catch {}
    setCurrentView('chat');
  };
  ...
  // Tracks whether the user has manually scrolled up — prevents auto-scroll overriding their position
  ...
  const [selectedNotebookTargetId, setSelectedNotebookTargetId] = useState('');
  const [saveNoteTitle, setSaveNoteTitle] = useState('');
  const [isLoadingNotebookTargets, setIsLoadingNotebookTargets] = useState(false);
  const [isSavingNotebookNote, setIsSavingNotebookNote] = useState(false);
  ...
  const toggleListening = async () => {
    ...
    setTimeout(async () => {
      try {
        ...
        // Visualizer is cosmetic — if it fails, recognition still works fine
      }
    }, 150);
  };
  ...
  const handleToggleReaction = async (msgId: string, type: 'like' | 'dislike') => {
    ...
    if (nextReactions[msgId] === type) {
      ...
      showToast(`Marked answer as ${type === 'like' ? 'helpful 👍' : 'needs improvement 👎'}`);
  ...
  const loadNotebookTargets = async () => {
    const token = localStorage.getItem('access_token');
    if (!token) {
      showToast('Please sign in before saving to notebooks.');
      return;
    }

    setIsLoadingNotebookTargets(true);
    try {
      const headers = { Authorization: `Bearer ${token}` };
      const [playlistsResponse, customFoldersResponse] = await Promise.all([
        fetch('/playlists', { headers }),
        fetch('/folders?all_folders=true', { headers }),
      ]);
      if (!playlistsResponse.ok) throw new Error('Failed to load notebooks.');

      const playlists = await playlistsResponse.json();
      const customFolders = customFoldersResponse.ok ? await customFoldersResponse.json() : [];
      ...
      for (const playlist of playlists) {
        ...
        const playlistNode = {
          id: playlistId,
          name: playlist.name || 'Untitled notebook',
          type: 'playlist' as const,
          children: [] as any[],
        };

        // Get custom folders belonging to this playlist
        const playlistFolders = customFolders.filter((cf: any) => String(cf.playlist_id) === playlistId);
        const folderCacheEntries: any[] = [];

        for (const cf of playlistFolders) {
          const name = String(cf.name || '').toLowerCase();
          // Skip system folders
          if (name === 'root' || name === 'notes' || name === 'media' || name === 'resources') continue;

          const dbParent = playlistFolders.find((f: any) => f.id === cf.parent_id);
          const parentIsSystem = dbParent && ['notes', 'media', 'resources', 'root'].includes(String(dbParent.name || '').toLowerCase());
          const effectiveParentId = parentIsSystem ? playlistId : (cf.parent_id || playlistId);
          ...
          // If effective parent is the playlist, add directly as child
          if (effectiveParentId === playlistId) {
            playlistNode.children.push(folderNode);
          }

          folderCacheEntries.push(folderNode);
        }

        // Build nested children for folders whose parent is another custom folder
        for (const entry of folderCacheEntries) {
          if (entry.parent_id !== playlistId) {
            const parentNode = folderCacheEntries.find((n: any) => n.id === entry.parent_id);
            if (parentNode) {
              parentNode.children.push(entry);
            } else {
              // Fallback: if parent not found, attach to playlist
              playlistNode.children.push(entry);
            }
          }
        }

        folderCache.set(playlistId, folderCacheEntries);
        treeData.push(playlistNode);
      }

      setNotebookTreeData(treeData);
      setNotebookFolderCache(folderCache);
    } catch (error: any) {
      console.error(error);
      showToast(error.message || 'Could load notebook folders.');
    } finally {
      setIsLoadingNotebookTargets(false);
    }
  };
  ...
  const handleSelectNotebookTarget = (type: 'playlist' | 'folder', id: string, playlistId: string) => {
    setSelectedNotebookTargetId(`${type}:${id}`);
  };

  const resolveTargetPlaylistId = (): string | null => {
    if (!selectedNotebookTargetId) return null;
    const [type, id] = selectedNotebookTargetId.split(':');
    if (type === 'playlist') return id;
    for (const playlist of notebookTreeData) {
      if (playlist.id === id) return playlist.id;
      const findInChildren = (nodes: any[]): string | null => {
        for (const node of nodes) {
          if (node.id === id) return playlist.id;
      ...
      if (findInChildren(playlist.children)) return playlist.id;
    }
    return null;
  };
    ...
    return nodes
      .map((node) => {
        if (node.type === 'playlist') {
          const playlistMatches = node.name.toLowerCase().includes(lowerQuery);
          const filteredChildren = node.children.filter((child: any) =>
            child.name.toLowerCase().includes(lowerQuery)
          );
          if (playlistMatches || filteredChildren.length > 0) {
            return {
              ...node,
              children: playlistMatches ? node.children : filteredChildren,
            };
          }
          return null;
        }
  ...
  const getSelectedPath = (): string => {
    if (!selectedNotebookTargetId) return '';
    const [type, id] = selectedNotebookTargetId.split(':');
    if (type === 'playlist') {
      const playlist = notebookTreeData.find((p) => p.id === id);
      return playlist ? `${playlist.name} / Notes` : '';
    }
    if (type === 'folder') {
      for (const playlist of notebookTreeData) {
        const findPath = (nodes: any[], path: string[]): string | null => {
          for (const node of nodes) {
            if (node.id === id) return [...path, node.name].join(' / ');
            if (node.children?.length) {
              const found = findPath(node.children, [...path, node.name]);
              if (found) return found;
            }
          }
          return null;
        };
        const path = findPath(playlist.children, [playlist.name]);
        if (path) return path;
      }
    }
    return '';
  ...
  const filteredNotebookTree = filterTreeBySearch(notebookTreeData, notebookSearchQuery);
  ...
  const handleSaveToNotes = () => {
    ...
    const newNote = {
      id: noteId,
      title: currentSession?.title || 'Chat note',
    ...
    showToast('Saved recommendation to Notes Desk! 📝');
    setIsNotesOpen(true);
  };

  const handleConfirmSaveToNotebook = async () => {
    const currentSession = chats.find(s => s.id === activeSessionId) || activeSession;
    const title = saveNoteTitle.trim() || currentSession?.title || 'Chat note';
    const token = localStorage.getItem('access_token');

    if (!currentSession || !selectedNotebookTargetId || !token) {
      showToast('Choose a notebook destination before saving.');
      return;
    }

    const [type, id] = selectedNotebookTargetId.split(':');
    const playlistId = resolveTargetPlaylistId();
    if (!playlistId) {
      showToast('Could not determine the target notebook.');
      return;
    }
    const folderId = type === 'folder' ? id : null;

    setIsSavingNotebookNote(true);
    try {
      const content = JSON.stringify(buildNotebookBlocksFromChat(currentSession));

      const createResponse = await fetch(`/playlists/${playlistId}/notes`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
      ...
      const createdNote = await createResponse.json().catch(() => ({}));
      if (!createResponse.ok) {
        throw new Error(createdNote?.detail || 'Failed to create notebook note.');
      }

      window.dispatchEvent(new CustomEvent('refresh-notebook-notes'));
      setIsSaveNoteModalOpen(false);
      showToast('Chat saved to notebook.');

      // Mark the chat session as saved to notebook
      try {
        await fetch(`/chat/sessions/${currentSession.id}/saved-to-notebook`, {
          method: 'PUT',
          headers: { Authorization: `Bearer ${token}` },
        });
        setChats(prev => prev.map(chat =>
          chat.id === currentSession.id ? { ...chat, savedToNotebook: true } : chat
        ));
      } catch {
        // Best effort — icon update is non-critical
      }
    } catch (error: any) {
      console.error(error);
      showToast(error.message || 'Failed to save chat to notebook.');
    } finally {
      setIsSavingNotebookNote(false);
    }
  };
    ...
    if (doc) {
      ...
      doc.write(`
        ...
        <html>
          ...
          <body>
            <h1>${title}</h1>
            <div class="meta">AI Optimization Grounded Document • Generated on ${new Date().toLocaleDateString()}</div>
            <div class="content">
              ${htmlContent}
            </div>
            <div class="footer">
              Ad Campaign Builder & Budget Optimizer • Elite SaaS Tool Desk
            </div>
            <script>
      ...
      `);
      doc.close();
      showToast('Preparing PDF/Print sheet! 📄');
    }
  };
    ...
    try {
      ...
      const hydratedChats = await Promise.all(
        chatSessions.map(async (session) => {
          ...
          const lastActivityAt = lastMsg?.created_at || session.created_at;

          return {
            id: session.id,
            title: session.title,
            date: formatSessionDate(lastActivityAt),
            timeAgo: formatSessionTimeAgo(lastActivityAt),
            messages,
            preview: preview.length > 120 ? `${preview.substring(0, 120)}...` : preview,
            lastActivityAt: lastActivityAt || undefined,
            savedToNotebook: session.saved_to_notebook || false,
          };
        }),
      );
    ...
    const interval = setInterval(() => {
      setChats(prev => prev.map(chat => {
        if (chat.lastActivityAt) {
          return { ...chat, timeAgo: formatSessionTimeAgo(chat.lastActivityAt) };
        }
        return chat;
      }));
    }, 60000);
  ...
  // Scroll to bottom — only when user hasn't manually scrolled up
  ...
  useEffect(() => {
    const el = document.getElementById('main-scroll-view');
    if (!el) return;
    const handleScroll = () => {
      // If user is more than 150px above the bottom, mark as scrolled up
  ...
  const handleSendMessage = async (text: string, overrideResources?: { id: string; title: string }[]) => {
    ...
    // User is sending a new message — reset scroll guard so we auto-scroll to their message
    ...
    try {
      ...
      const withAiMsg = updatedChats.map(c => {
        if (c.id === sessionId) {
          ...
          const msgs = c.messages.map(m => {
            ...
            return {
              ...
              details: finalPayload ? {
                ...
                hallucinationCount: finalPayload.hallucinations?.length ?? null,
                hallucinationCheckPassed: finalPayload.hallucinations != null ? finalPayload.hallucinations.length === 0 : null,
                sourceCount: finalPayload.sources?.length ?? null,
                retrievalStrategy: finalPayload.retrieval_strategy,
  ...
  return (
    <div className="w-full h-full flex justify-center items-center font-sans selection:bg-slate-200 dark:selection:bg-indigo-400/25 dark:selection:text-white relative overflow-hidden">
      ...
      {/* Main App Container */}
      <div className={`w-full h-full flex flex-col relative overflow-hidden ${isDarkMode ? '' : 'bg-white/40'}`}>
        ...
        <header className="flex justify-between items-center px-6 sm:px-10 py-6 z-20 shrink-0">
          <div className="flex items-center gap-3 sm:gap-4">
            <button
              ...
              className={`flex items-center justify-center w-11 h-11 rounded-full transition-all duration-300 hover:shadow-sm ${
                isDarkMode ? 'border border-white/10 hover:bg-white/10' : 'border border-gray-200 hover:bg-gray-50'
              }`}
              title="New Chat"
            >
              <Plus size={18} strokeWidth={2.5} className={isDarkMode ? 'text-gray-300' : 'text-gray-500'} />
            </button>
            ...
            <div className="relative">
              <button
                ...
                className={`flex items-center gap-2 px-5 py-2.5 rounded-full transition-all duration-300 hover:shadow-sm ${
                  isDarkMode
                    ? `border border-white/10 hover:bg-white/10 ${isHeaderExportOpen ? 'bg-white/10 border-white/20' : 'bg-white/5'}`
                    : `border border-gray-200 hover:bg-gray-50 ${isHeaderExportOpen ? 'bg-gray-50 border-gray-300' : 'bg-white'}`
                }`}
                title="Export Whole Chat"
              >
                <Download size={16} strokeWidth={2.5} className={`${isDarkMode ? 'text-gray-300' : 'text-gray-500'} animate-pulse`} />
                <span className={`text-sm font-medium ${isDarkMode ? 'text-gray-200' : 'text-gray-700'}`}>Export</span>
                <ChevronDown size={14} className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'} transition-transform duration-200 ${isHeaderExportOpen ? 'rotate-180' : ''}`} />
              </button>
                ...
                {isHeaderExportOpen && activeSession && (
                  <>
                    ...
                    <motion.div
                      ...
                      className={`absolute left-0 mt-2 w-64 rounded-2xl p-2.5 z-50 flex flex-col gap-1.5 ${
                        isDarkMode
                          ? 'bg-neutral-800/95 border border-white/10 shadow-[0_10px_40px_rgb(0,0,0,0.4)] backdrop-blur-md'
                          : 'bg-white border border-gray-200/80 shadow-[0_10px_40px_rgb(0,0,0,0.08)]'
                      }`}
                    >
                      <div className={`px-3 py-1 text-[11px] font-bold uppercase tracking-widest pb-2 mb-1.5 ${
                        isDarkMode ? 'text-gray-400 border-b border-white/10' : 'text-gray-500 border-b border-gray-100'
                      }`}>
                        Export Whole Chat
                      </div>
                      <button
                        onClick={handleExportWholeChatMarkdown}
                        className={`flex items-center gap-3 w-full text-left px-3.5 py-2.5 rounded-xl text-xs font-semibold transition-colors ${
                          isDarkMode ? 'text-gray-200 hover:bg-white/10 hover:text-white' : 'text-gray-700 hover:bg-gray-50 hover:text-black'
                        }`}
                      >
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            <button
              ...
              className={`flex items-center justify-center w-11 h-11 rounded-lg transition-all duration-300 hover:shadow-sm ${
                isDarkMode
                  ? `border border-white/10 ${currentView === 'history' ? 'bg-white/10 shadow-inner border-white/20' : 'hover:bg-white/10 bg-white/5'}`
                  : `border border-gray-200 ${currentView === 'history' ? 'bg-gray-100 shadow-inner border-gray-300' : 'hover:bg-gray-50 bg-white'}`
              }`}
              title="Toggle History View"
            >
              <PanelLeft size={18} strokeWidth={2} className={isDarkMode ? 'text-gray-300' : 'text-gray-500'} />
            </button>
            <button
              onClick={handleSaveToNotes}
              className={`flex items-center gap-2 px-5 py-2.5 rounded-full transition-all duration-300 hover:shadow-sm ${
                activeSession?.savedToNotebook
                  ? isDarkMode ? 'border border-white/20 bg-white/10' : 'border border-gray-300 bg-gray-100'
                  : isDarkMode ? 'border border-white/10 hover:bg-white/10 bg-white/5' : 'border border-gray-200 hover:bg-gray-50 bg-white'
              }`}
              title={activeSession?.savedToNotebook ? "Saved to notebook" : "Save Current Recommendation to Notes"}
            >
              <Bookmark
                size={18}
                strokeWidth={2}
                className={activeSession?.savedToNotebook
                  ? isDarkMode ? 'text-white fill-white' : 'text-gray-900 fill-gray-900'
                  : isDarkMode ? 'text-gray-300' : 'text-gray-500'
                }
              />
              <span className={`text-sm font-medium ${isDarkMode ? 'text-gray-200' : 'text-gray-800'}`}>
                {activeSession?.savedToNotebook ? 'Saved' : 'Save to Note'}
              </span>
            </button>
          </div>
        </header>
              ...
              {isVoicePanelOpen ? (
                ...
                >
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center min-w-0 overflow-hidden gap-1.5">
                    ...
                    {/* Live transcript text — shows as speech is detected */}
                    {voiceDraft ? (
          ...
          {isSaveNoteModalOpen && (
            <>
              <motion.div
                ...
                onClick={() => !isSavingNotebookNote && setIsSaveNoteModalOpen(false)}
              />
              <motion.div
                ...
                className={`absolute left-1/2 top-1/2 z-50 w-[min(92vw,520px)] -translate-x-1/2 -translate-y-1/2 rounded-[28px] border p-5 shadow-2xl ${
                  isDarkMode
                    ? 'border-white/10 bg-[#2b2d31]/95 text-gray-100 shadow-black/40'
                    : 'border-white/70 bg-white/95 text-slate-900 shadow-slate-300/40'
                } backdrop-blur-xl`}
              >
                <div className="mb-5 flex items-start justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-2">
                      <Bookmark size={17} className={isDarkMode ? 'text-indigo-300' : 'text-indigo-600'} />
                      <h2 className="text-sm font-bold">Save chat to notebook</h2>
                    </div>
                  ...
                  <button
                    type="button"
                    disabled={isSavingNotebookNote}
                    onClick={() => setIsSaveNoteModalOpen(false)}
                    className={`rounded-full p-2 transition-colors ${
                      isDarkMode ? 'text-gray-400 hover:bg-white/10 hover:text-white' : 'text-slate-400 hover:bg-slate-100 hover:text-slate-700'
                    } disabled:opacity-50`}
                    title="Cancel"
                  >
                    <X size={17} />
                ...
                <div className="space-y-4">
                  <label className="block">
                    <span className={`mb-1.5 block text-[11px] font-bold uppercase tracking-wider ${isDarkMode ? 'text-gray-400' : 'text-slate-500'}`}>
                      Note name
                    </span>
                    <input
                      value={saveNoteTitle}
                      onChange={(event) => setSaveNoteTitle(event.target.value)}
                      className={`w-full rounded-2xl border px-4 py-3 text-sm outline-none transition-colors ${
                        isDarkMode
                          ? 'border-white/10 bg-white/5 text-white placeholder:text-gray-500 focus:border-indigo-300/50'
                  ...
                  <label className="block">
                    ...
                    {/* Breadcrumb */}
                    {selectedNotebookTargetId && (
                      <div className={`mb-2 flex items-center gap-1 text-xs ${isDarkMode ? 'text-indigo-300' : 'text-indigo-600'}`}>
                        <Bookmark size={12} />
                    ...
                    {/* Search input */}
                    <div className="relative">
                      <Search size={14} className={`absolute left-3 top-1/2 -translate-y-1/2 ${isDarkMode ? 'text-gray-500' : 'text-slate-400'}`} />
                      <input
                        type="text"
                        value={notebookSearchQuery}
                        onChange={(e) => setNotebookSearchQuery(e.target.value)}
                        placeholder="Search notebooks and folders..."
                        className={`w-full rounded-2xl border pl-9 pr-4 py-2.5 text-sm outline-none transition-colors ${
                          isDarkMode
                            ? 'border-white/10 bg-white/5 text-white placeholder:text-gray-500 focus:border-indigo-300/50'
                            : 'border-slate-200 bg-white text-slate-900 placeholder:text-slate-400 focus:border-indigo-300'
                        }`}
                    ...
                    {/* Tree */}
                    <div className={`mt-2 max-h-[240px] overflow-y-auto rounded-2xl border p-2 ${
                      isDarkMode
                        ? 'border-white/10 bg-[#1e1f22]'
                        : 'border-slate-200 bg-slate-50'
                    }`}>
                      {isLoadingNotebookTargets ? (
                        <div className="flex items-center justify-center py-6 gap-2">
                          <Loader2 size={16} className="animate-spin text-indigo-500" />
                          <span className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-slate-500'}`}>Loading notebooks...</span>
                        </div>
                      ) : filteredNotebookTree.length === 0 ? (
                        <div className="py-6 text-center">
                          <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-slate-500'}`}>
                            {notebookSearchQuery ? 'No results match your search.' : 'No notebooks found.'}
                          </p>
                        </div>
                      ) : (
                        filteredNotebookTree.map((playlist) => (
                          <div key={playlist.id} className="mb-1">
                            {/* Playlist row */}
                            <button
                              type="button"
                              onClick={() => {
                                handleSelectNotebookTarget('playlist', playlist.id, playlist.id);
                              ...
                              className={`w-full flex items-center gap-2 px-3 py-2 rounded-xl text-left text-sm transition-colors ${
                                selectedNotebookTargetId === `playlist:${playlist.id}`
                                  ? isDarkMode
                            ...
                            >
                              {playlist.children.length > 0 && (
                                <ChevronDown
                                  size={14}
                                  className={`shrink-0 transition-transform ${expandedPlaylistIds.has(playlist.id) ? 'rotate-0' : '-rotate-90'} ${isDarkMode ? 'text-gray-500' : 'text-slate-400'}`}
                                />
                              )}
                              {playlist.children.length === 0 && <span className="w-[14px] shrink-0" />}
                            ...
                            {expandedPlaylistIds.has(playlist.id) && playlist.children.length > 0 && (
                              <div className="ml-4 mt-0.5">
                                {playlist.children.map((folder: any) => (
                                  <div key={folder.id}>
                                    <button
                                      type="button"
                                      onClick={() => {
                                        handleSelectNotebookTarget('folder', folder.id, playlist.id);
                                      ...
                                      className={`w-full flex items-center gap-2 px-3 py-1.5 rounded-lg text-left text-xs transition-colors ${
                                        selectedNotebookTargetId === `folder:${folder.id}`
                                          ? isDarkMode
                                            ? 'bg-indigo-500/20 text-indigo-300'
                                            : 'bg-indigo-100 text-indigo-700'
                                          : isDarkMode
                                    ...
                                    >
                                      {folder.children?.length > 0 ? (
                                        <ChevronDown
                                          size={12}
                                          className={`shrink-0 transition-transform ${expandedFolderIds.has(folder.id) ? 'rotate-0' : '-rotate-90'} ${isDarkMode ? 'text-gray-500' : 'text-slate-400'}`}
                                        />
                                      ) : (
                                      ...
                                      <div className="ml-4">
                                        {folder.children.map((sub: any) => (
                                          <button
                                            key={sub.id}
                                            type="button"
                                            onClick={() => handleSelectNotebookTarget('folder', sub.id, playlist.id)}
                                            className={`w-full flex items-center gap-2 px-3 py-1.5 rounded-lg text-left text-xs transition-colors ${
                                              selectedNotebookTargetId === `folder:${sub.id}`
                                                ? isDarkMode
                                                  ? 'bg-indigo-500/20 text-indigo-300'
                                                  : 'bg-indigo-100 text-indigo-700'
                                                : isDarkMode
                ...
                <div className={`mt-5 flex items-center justify-end gap-2 border-t pt-4 ${isDarkMode ? 'border-white/10' : 'border-slate-100'}`}>
                  <button
                    type="button"
                    disabled={isSavingNotebookNote}
                    onClick={() => setIsSaveNoteModalOpen(false)}
                    className={`rounded-full px-4 py-2 text-sm font-semibold transition-colors ${
                      isDarkMode ? 'text-gray-300 hover:bg-white/10' : 'text-slate-600 hover:bg-slate-100'
                    } disabled:opacity-50`}
                  >
                    Cancel
                  </button>
                  <button
                    type="button"
                    disabled={isSavingNotebookNote || isLoadingNotebookTargets || !selectedNotebookTargetId}
                    onClick={handleConfirmSaveToNotebook}
                  ...
                  >
                    {isSavingNotebookNote && <Loader2 size={15} className="animate-spin" />}
                    Save
                  </button>
                </div>
              </motion.div>
        ...
        {/* RAG Focus Search Modal — Premium Redesign */}
        <AnimatePresence>
          {isSearchModalOpen && (
            <>
              {/* Backdrop */}
              <motion.div
                ...
                style={{ background: isDarkMode ? 'rgba(12,14,18,0.62)' : 'rgba(244,247,252,0.55)', backdropFilter: 'blur(18px)' }}
              >
                {/* Modal Card */}
                <motion.div
                  ...
                  style={{
                    background: isDarkMode ? 'rgba(35,36,40,0.94)' : 'rgba(255,255,255,0.78)',
                    borderRadius: '28px',
                    border: isDarkMode ? '1px solid rgba(255,255,255,0.06)' : '1px solid rgba(255,255,255,0.7)',
                    boxShadow: isDarkMode ? '0 32px 80px rgba(0,0,0,0.45), 0 2px 8px rgba(0,0,0,0.18)' : '0 32px 80px rgba(148,163,184,0.18), 0 2px 8px rgba(255,255,255,0.35)',
                    maxHeight: '82vh',
                  }}
                >

                  {/* ── HEADER ── */}
                  <div className="flex items-start justify-between px-6 pt-6 pb-5 shrink-0" style={{ borderBottom: isDarkMode ? '1px solid rgba(255,255,255,0.06)' : '1px solid rgba(226,232,240,0.9)' }}>
                    <div className="flex items-start gap-3">
                      {/* Icon bubble */}
                      <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 shadow-sm mt-0.5 ${isDarkMode ? 'bg-[#111827] border border-white/10' : 'bg-white/75 border border-white/70'}`}>
                        <Paperclip size={16} className={isDarkMode ? 'text-white' : 'text-slate-700'} strokeWidth={2} />
                      </div>
                      <div>
                        <h2 className={`text-[15px] font-bold leading-tight tracking-tight ${isDarkMode ? 'text-[#f2f3f5]' : 'text-slate-900'}`}>
                          Focus AI on Files
                        </h2>
                        <p className={`mt-0.5 text-[11px] leading-snug ${isDarkMode ? 'text-[#9ca3af]' : 'text-gray-400'}`}>
                          Search and select up to&nbsp;<strong className={isDarkMode ? 'text-[#dbdee1]' : 'text-gray-600'}>5 resources</strong>&nbsp;to ground the AI context.
                        </p>
                      </div>
                    </div>
                    <button
                      type="button"
                      onClick={() => setIsSearchModalOpen(false)}
                      className={`w-7 h-7 rounded-full flex items-center justify-center transition-colors shrink-0 mt-0.5 ${isDarkMode ? 'text-[#9ca3af] hover:text-white' : 'text-gray-400 hover:text-gray-700'}`}
                      style={{ background: isDarkMode ? 'rgba(255,255,255,0.06)' : '#F2F2F4' }}
                      title="Close"
                    >
                  ...
                  <AnimatePresence>
                    {selectedResources.length > 0 && (
                      ...
                      >
                        <div className="px-6 py-3 flex flex-col gap-2" style={{ background: isDarkMode ? 'rgba(255,255,255,0.03)' : '#F7F7F9', borderBottom: isDarkMode ? '1px solid rgba(255,255,255,0.06)' : '1px solid #EBEBED' }}>
                          <span className={`text-[9px] font-extrabold uppercase tracking-[0.12em] ${isDarkMode ? 'text-[#9ca3af]' : 'text-gray-400'}`}>
                  ...
                  {/* ── SEARCH INPUT ── */}
                  <div className="px-5 py-4 shrink-0 flex items-center gap-2.5" style={{ borderBottom: isDarkMode ? '1px solid rgba(255,255,255,0.06)' : '1px solid #EBEBED' }}>
                    <div className="relative flex-grow">
                      <Search size={15} className={`absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none ${isDarkMode ? 'text-[#9ca3af]' : 'text-gray-400'}`} />
                      <input
                        type="text"
                        autoFocus
                        value={searchQuery}
                        ...
                        className={`w-full rounded-xl pl-9 pr-9 py-2.5 text-[13px] outline-none transition-all ${isDarkMode ? 'text-[#f2f3f5] placeholder:text-[#9ca3af]' : 'text-gray-800 placeholder:text-gray-400'}`}
                        style={{
                          background: isDarkMode ? 'rgba(255,255,255,0.05)' : '#F2F2F5',
                          border: isDarkMode ? '1.5px solid rgba(255,255,255,0.06)' : '1.5px solid transparent',
                        }}
                        onFocus={(e) => { e.currentTarget.style.border = isDarkMode ? '1.5px solid rgba(99,102,241,0.45)' : '1.5px solid #C7C7D0'; e.currentTarget.style.background = isDarkMode ? 'rgba(255,255,255,0.07)' : '#fff'; }}
                        onBlur={(e) => { e.currentTarget.style.border = isDarkMode ? '1.5px solid rgba(255,255,255,0.06)' : '1.5px solid transparent'; e.currentTarget.style.background = isDarkMode ? 'rgba(255,255,255,0.05)' : '#F2F2F5'; }}
                      />
                      {isSearching ? (
                        ...
                        <button
                          ...
                          className={`absolute right-3 top-1/2 -translate-y-1/2 transition-colors ${isDarkMode ? 'text-[#9ca3af] hover:text-white' : 'text-gray-400 hover:text-gray-600'}`}
                        >
                          <X size={13} strokeWidth={2.5} />
                        </button>
                      ) : null}
                  ...
                  <div className="flex-grow overflow-y-auto px-4 py-3 space-y-1.5 no-scrollbar">
                    {!searchQuery.trim() ? (
                      ...
                      <div className="flex flex-col items-center justify-center py-14 text-center gap-2">
                        <div className="w-12 h-12 rounded-2xl flex items-center justify-center mb-1" style={{ background: isDarkMode ? 'rgba(255,255,255,0.05)' : '#F2F2F5' }}>
                          <Search size={20} className={isDarkMode ? 'text-[#9ca3af]' : 'text-gray-400'} />
                        </div>
                        <p className={`text-[13px] font-semibold ${isDarkMode ? 'text-[#dbdee1]' : 'text-gray-600'}`}>Search your library</p>
                        <p className={`text-[11px] max-w-[240px] leading-relaxed ${isDarkMode ? 'text-[#9ca3af]' : 'text-gray-400'}`}>
                      ...
                      <div className="flex flex-col items-center justify-center py-14 text-center gap-2">
                        <div className="w-12 h-12 rounded-2xl flex items-center justify-center mb-1" style={{ background: isDarkMode ? 'rgba(255,255,255,0.05)' : '#F2F2F5' }}>
                          <Search size={20} className={isDarkMode ? 'text-[#6b7280]' : 'text-gray-300'} />
                        </div>
                        <p className={`text-[13px] font-semibold ${isDarkMode ? 'text-[#dbdee1]' : 'text-gray-600'}`}>No results found</p>
                        <p className={`text-[11px] max-w-[240px] leading-relaxed ${isDarkMode ? 'text-[#9ca3af]' : 'text-gray-400'}`}>
                      ...
                      searchResults.map((resource) => {
                        ...
                        return (
                          <motion.button
                            ...
                            style={isSelected ? {
                              background: isDarkMode ? '#111827' : '#0F172A',
                              border: isDarkMode ? '1.5px solid rgba(99,102,241,0.35)' : '1.5px solid #0F172A',
                              boxShadow: isDarkMode ? '0 4px 14px rgba(0,0,0,0.35)' : '0 4px 14px rgba(15,23,42,0.25)',
                            } : {
                              background: isDarkMode ? 'rgba(255,255,255,0.03)' : '#FFFFFF',
                              border: isDarkMode ? '1.5px solid rgba(255,255,255,0.06)' : '1.5px solid #EBEBED',
                            }}
                          >
                            {/* Checkbox */}
                            <div
                              className="shrink-0 w-4 h-4 rounded flex items-center justify-center transition-all"
                              style={isSelected ? { background: '#fff', border: '2px solid #fff' } : { border: isDarkMode ? '2px solid rgba(156,163,175,0.5)' : '2px solid #D1D5DB', background: 'transparent' }}
                            >
                            ...
                            <div className="flex-1 min-w-0">
                              <p className={`text-[13px] font-semibold truncate leading-tight ${isSelected ? 'text-white' : isDarkMode ? 'text-[#f2f3f5]' : 'text-gray-800'}`}>
                                {resource.title}
                              </p>
                              {resource.description && (
                                <p className={`text-[10px] truncate mt-0.5 ${isSelected ? 'text-slate-400' : isDarkMode ? 'text-[#9ca3af]' : 'text-gray-400'}`}>
                                  {resource.description}
                                </p>
                              )}
                            </div>
                            ...
                            <span
                              ...
                              style={isSelected
                                ...
                                : isDarkMode
                                  ? { background: isVideo ? 'rgba(245,158,11,0.14)' : 'rgba(99,102,241,0.14)', color: isVideo ? '#fbbf24' : '#c7d2fe' }
                  ...
                  <div
                    ...
                    style={{ borderTop: isDarkMode ? '1px solid rgba(255,255,255,0.06)' : '1px solid #EBEBED', background: isDarkMode ? 'rgba(255,255,255,0.03)' : '#F7F7F9' }}
                  >
                    <div className="flex items-center gap-2">
                      <div className="flex gap-1">
                        {Array.from({ length: 5 }).map((_, i) => (
                          <div
                            ...
                            style={{ background: i < selectedResources.length ? (isDarkMode ? '#c7d2fe' : '#0F172A') : (isDarkMode ? 'rgba(156,163,175,0.35)' : '#D1D5DB') }}
                          />
                        ))}
                      </div>
                      <span className={`text-[11px] font-medium ${isDarkMode ? 'text-[#9ca3af]' : 'text-gray-500'}`}>
                    ...
                    <button
                      ...
                      style={{
                        background: isDarkMode ? '#5865f2' : '#0F172A',
                        borderRadius: '12px',
                        padding: '8px 20px',
                        boxShadow: isDarkMode ? '0 8px 18px rgba(88,101,242,0.28)' : '0 4px 12px rgba(15,23,42,0.30)',
                      }}
                    >
    ...
    >
      ...
      <p className="text-gray-500 mt-3 text-base sm:text-lg text-center">
        {suggestions && suggestions.length > 0
          ? 'Here are resources from your library — click to explore.'
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mt-12 w-full max-w-[900px]">
        {isLoading ? (
          ...
          cards.map((sug: any, i: number) => {
            ...
            const cardKind = sug.kind || (sug.icon === 'video' ? 'video' : 'doc');
            const badgeLabel = cardKind === 'video' ? 'Video' : cardKind === 'audio' ? 'Audio' : 'PDF';
            const badgeColors = cardKind === 'video'
              ...
              >
                ...
                {isDynamic ? (
                  <div className="w-9 h-9 rounded-xl mb-4 flex items-center justify-center"
                    style={{ background: badgeColors.background }}
                  >
                    {cardKind === 'video'
                      ? <BarChart2 size={18} strokeWidth={2} style={{ color: badgeColors.color }} />
                      : cardKind === 'audio'
                        ? <Mic size={18} strokeWidth={2} style={{ color: badgeColors.color }} />
                        : <ReceiptText size={18} strokeWidth={2} style={{ color: badgeColors.color }} />
                    }
                  </div>
                ) : (
      ...
      <div className="flex flex-col gap-8 flex-grow pb-12">
        {session.messages.map((msg, msgIndex) => (
          ...
          >
            ...
            {msg.role === 'assistant' && msg.content && (
              <div className="flex gap-3 w-full items-start group/msg">
                {/* AI icon */}
                <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 mt-0.5 backdrop-blur-sm ${isDarkMode ? 'bg-white/10 border border-white/10 shadow-sm' : 'bg-white/70 border border-white/80 shadow-[0_12px_24px_-16px_rgba(148,163,184,0.55)]'}`}>
                  <Sparkles size={14} className={isDarkMode ? 'text-gray-300' : 'text-indigo-500'} />
                </div>
                {/* AI body — no box, just text */}
                <div className="flex-1 min-w-0">
                  <div className={`rounded-[30px] rounded-tl-none px-6 py-5 backdrop-blur-md ${
                    isDarkMode
                      ? 'border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.08),rgba(255,255,255,0.04))] shadow-[0_24px_60px_-28px_rgba(0,0,0,0.55)]'
                      : 'border border-white/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.72),rgba(255,255,255,0.5))] shadow-[0_24px_60px_-28px_rgba(148,163,184,0.35)]'
                  }`}>
                    <div className={`mb-4 flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.16em] ${isDarkMode ? 'text-indigo-200/85' : 'text-indigo-500/80'}`}>
                      <span className={`flex h-5 w-5 items-center justify-center rounded-md ${isDarkMode ? 'bg-indigo-400/15' : 'bg-indigo-500/12'}`}>
                        <Sparkles size={11} className={isDarkMode ? 'text-indigo-200' : 'text-indigo-500'} />
                      </span>
                      <span>RAG Assistant Response</span>
                    </div>

                    <div className={`text-[15px] leading-relaxed ${isDarkMode ? 'text-gray-100' : 'text-slate-700'}`}>
                      <TypewriterMessage
                        content={msg.content}
                        msgId={msg.id}
                  ...
                  {/* Action bar — latency info left, action buttons right */}
                  <div className={`mt-4 flex items-center justify-between gap-3 pt-3 ${isDarkMode ? 'border-t border-white/10' : 'border-t border-slate-200/70'}`}>
                    {/* Left side — latency / model info */}
                    <div className="flex items-center gap-3 flex-wrap">
                      {msg.details?.processingTimeMs != null && (
                        <span className={`inline-flex items-center gap-1 text-[10px] font-medium ${isDarkMode ? 'text-gray-400' : 'text-slate-500'}`}>
                          <Clock size={11} />
                    ...
                    <button
                      ...
                      className={`p-1.5 rounded-lg transition-colors cursor-pointer ${isDarkMode ? 'hover:bg-white/10 hover:text-white text-gray-400' : 'hover:bg-slate-100 hover:text-slate-800 text-slate-400'} ${reactions[msg.id] === 'like' ? 'text-emerald-400 bg-emerald-500/10' : ''}`}
                      title="Helpful"
                    >
                    ...
                    <button
                      ...
                      className={`p-1.5 rounded-lg transition-colors cursor-pointer ${isDarkMode ? 'hover:bg-white/10 hover:text-white text-gray-400' : 'hover:bg-slate-100 hover:text-slate-800 text-slate-400'} ${reactions[msg.id] === 'dislike' ? 'text-rose-400 bg-rose-500/10' : ''}`}
                      title="Not helpful"
                    >
                    ...
                    <button
                      ...
                      className={`p-1.5 rounded-lg transition-colors cursor-pointer ${isDarkMode ? 'hover:bg-white/10 hover:text-white text-gray-400' : 'hover:bg-slate-100 hover:text-slate-800 text-slate-400'}`}
                      title="Regenerate"
                    >
                      <RefreshCw size={13} />
                    </button>
                    ...
                    <div className="relative inline-block">
                      <button
                        ...
                        className={`p-1.5 rounded-lg transition-colors cursor-pointer ${isDarkMode ? 'hover:bg-white/10 hover:text-white text-gray-400' : 'hover:bg-slate-100 hover:text-slate-800 text-slate-400'} ${openDropdownMsgId === msg.id ? (isDarkMode ? 'bg-white/10 text-white' : 'bg-slate-100 text-slate-800') : ''}`}
                        title="More options"
                      >
                        ...
                        {openDropdownMsgId === msg.id && (
                          <>
                            ...
                            <motion.div
                              ...
                              transition={{ duration: 0.12 }}
                              className={`absolute left-0 mt-1 w-44 rounded-xl p-1 z-50 flex flex-col backdrop-blur-md shadow-lg ${
                                isDarkMode
                            ...
                            >
                              <button
                                ...
                                className={`flex items-center gap-2 text-left w-full px-2.5 py-1.5 rounded-lg text-[11px] font-semibold transition-colors ${
                                  isDarkMode
                                    ? 'text-gray-200 hover:bg-white/10 hover:text-white'
                                    : 'text-slate-700 hover:bg-slate-100 hover:text-slate-900'
                                }`}
                              >
                                <FileDown size={13} className={isDarkMode ? 'text-gray-400' : 'text-slate-500'} />
                                <span>Export to Markdown</span>
                              </button>
                              <button
                    ...
                    <span className={`text-[10px] font-medium ${isDarkMode ? 'text-gray-500' : 'text-slate-500'}`}>{msg.timestamp}</span>
                    </div>
                  </div>
                  </div>
                </div>
            ...
            {msg.role === 'user' && (
              <div className="flex flex-col items-end max-w-[75%]">
                <div className={`rounded-3xl rounded-br-md px-5 py-3.5 text-[15px] leading-relaxed font-normal shadow-sm backdrop-blur-sm ${
                  isDarkMode
                    ? 'bg-white/12 border border-white/10 text-white'
                    : 'bg-white/60 border border-white/80 text-slate-700 shadow-[0_16px_40px_-26px_rgba(148,163,184,0.45)]'
                }`}>
                  {msg.content}
                </div>
                <span className={`mt-1.5 pr-1 text-[10px] font-medium ${isDarkMode ? 'text-gray-400' : 'text-slate-500'}`}>{msg.timestamp}</span>
              </div>
            )}
          </motion.div>
        ))}

        {/* Live generation loading indicator — only shown while AI hasn't started producing content yet */}
        {isGenerating && session.messages[session.messages.length - 1]?.content === '' && (
          ...
          >
            ...
            <div className="bg-white/10 border border-white/10 rounded-3xl rounded-tl-none p-5 shadow-sm backdrop-blur-sm">
...
function ChatScrollTimeline({
  ...
  return (
    <div className={`absolute right-5 sm:right-6 top-1/2 -translate-y-1/2 flex flex-col items-center gap-3 z-40 p-1.5 py-3 rounded-full shadow-md ${
      isDarkMode
      ...
      <button
        ...
        className={`transition-colors p-1 rounded-full ${
          isDarkMode ? 'text-gray-400 hover:text-white hover:bg-white/10' : 'text-gray-400 hover:text-slate-800 hover:bg-slate-50'
        }`}
        title="Scroll Up"
      >
      ...
      <div className="flex flex-col items-center gap-2 py-1 select-none">
        {messages.map((msg, index) => {
          ...
          return (
            <div
              key={msg.id}
              className="relative flex items-center justify-center cursor-pointer group py-0.5 w-6"
            ...
            >
              ...
              <div
                className={`h-[3px] rounded-full transition-all duration-300 ease-out relative z-10 origin-center ${hoveredMsgId === msg.id
                  ? `w-3.5 scale-x-[1.7] ${isDarkMode ? 'bg-white shadow-sm' : 'bg-black shadow-sm'}`
                  : isAssistant
                    ? (isDarkMode ? 'w-2 bg-white/55 group-hover:bg-white/80' : 'w-2 bg-gray-300 group-hover:bg-gray-500')
                    : (isDarkMode ? 'w-3.5 bg-white/70 group-hover:bg-white' : 'w-3.5 bg-gray-400 group-hover:bg-gray-600')
                  }`}
              />
              ...
              <AnimatePresence>
                {hoveredMsgId === msg.id && (
                  <motion.div
                    ...
                    transition={{ duration: 0.12 }}
                    className={`absolute right-10 top-1/2 -translate-y-1/2 w-64 rounded-2xl p-3.5 z-50 text-left pointer-events-none ${
                      isDarkMode
                        ? 'bg-neutral-800/95 text-white shadow-[0_8px_30px_rgb(0,0,0,0.4)] backdrop-blur-md'
                        : 'bg-white/95 text-slate-800 shadow-[0_8px_30px_rgb(0,0,0,0.12)] backdrop-blur-md'
                    }`}
                  >
                    <div className="flex items-center gap-1.5 mb-1.5">
                      {isAssistant ? (
                        <Sparkles size={11} className={`${isDarkMode ? 'text-purple-600' : 'text-purple-600'} animate-pulse`} />
                      ) : (
                        <div className="w-1.5 h-1.5 bg-blue-500 rounded-full" />
                      )}
                      <span className={`text-[10px] uppercase tracking-wider font-bold ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                        {senderLabel}
                      </span>
                    </div>
                    <p className={`text-xs leading-relaxed font-sans line-clamp-3 ${isDarkMode ? 'text-gray-300' : 'text-slate-700'}`}>
                      {shortPreview}
                    </p>
                    <span className={`text-[9px] mt-2 block font-mono text-right ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`}>
                      {msg.timestamp}
                    </span>
                  </motion.div>
                )}
