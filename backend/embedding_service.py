...
def embed_texts(
    ...
    """Embed many texts using batched API calls (faster + cheaper than one-per-call).

    Order-preserving: ``results[i]`` always corresponds to ``texts[i]``.
    - Empty/whitespace texts map to a zero vector (matching ``embed_text``).
    - If a batch API call fails, that batch falls back to per-item ``embed_text``,
      so this can never behave worse than the legacy one-at-a-time path.
    - Zero vectors use the dimension detected from real embeddings in this run
      (falls back to 3072 only if nothing else is available).
    """
    if not texts:
        return []
    ...
    for start in range(0, len(pending), step):
        ...
        try:
            response = _client.embeddings.create(model=_model, input=inputs)
            data = sorted(response.data, key=lambda d: getattr(d, "index", 0))
            if len(data) != len(inputs):
                raise ValueError(
                    f"embedding count mismatch: got {len(data)} for {len(inputs)} inputs"
                )
            ...
            # Safe fallback: embed this batch one item at a time (legacy behavior).
            logger.warning(
                f"Batch embedding failed ({e}); falling back to per-item for this batch of {len(inputs)}"
            )
            for i, t in batch:
                vec = embed_text(
                    t, user_id=user_id, resource_id=resource_id, feature=feature
                )
                results[i] = vec
                if vec:
                    detected_dim = len(vec)

    dim = detected_dim or 3072
...
def store_resource_embeddings(
    ...
    def _embed_texts_compat(texts: list[str]):
        # Batched embedding for the changed chunks. embed_texts() already falls back
        # to per-item embed_text() internally if a batch call fails, so this is safe.
        return embed_texts(
            texts,
            user_id=user_id,
            resource_id=resource_id,
    ...
    # Phase 3: pre-compute embeddings for the changed chunks in batched API calls.
    # Chunks whose content is unchanged reuse their existing vector (no API call);
    # the rest are embedded together in one batched request per `batch_size` instead
    # of one HTTP round-trip each. Positions are keyed by index into changed_records,
    # so each vector stays aligned to its exact chunk.
    embeddings_by_pos: dict[int, list] = {}
    batch_texts: list[str] = []
    batch_positions: list[int] = []
    for _idx, _record in enumerate(changed_records):
        _bucket = reusable_embeddings.get(_record["signature"]) or []
        if _bucket:
            embeddings_by_pos[_idx] = _bucket.pop()
            reused_count += 1
        else:
            batch_texts.append(
                _build_embedding_text_compat(transcript, _record["content"])
            )
            batch_positions.append(_idx)

    if batch_texts:
        logger.info(f"STORE EMBEDDINGS: Batch-embedding {len(batch_texts)} new chunks")
        batch_vectors = _embed_texts_compat(batch_texts)
        for _pos, _vector in zip(batch_positions, batch_vectors):
            embeddings_by_pos[_pos] = _vector
            embedded_count += 1
    ...
    for record_idx, record in enumerate(changed_records):
        ...
        # Vector was pre-computed above (reused from the pool, or freshly batch-embedded).
        embedding = embeddings_by_pos.get(record_idx)
        if embedding is None:
            # Safety net: should not happen, but never store a chunk without a vector.
    ...
    # Phase 3: the resource's content changed, so any cached answers for it are now
    # stale. Drop them here (best-effort) so the next question re-runs retrieval.
    try:
        from services.cache_service import invalidate_resource_cache
        invalidate_resource_cache(db, resource_id)
    except Exception as e:
        logger.warning(f"Failed to invalidate semantic cache for {resource_id}: {e}")
